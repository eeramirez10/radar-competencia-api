...
import { CompetitorDataset, type CompetitorDatasetStore } from '../src/competitor-dataset.js'
import { CustomerSalesCache, type CustomerCacheFile } from '../src/cache.js'
import { clearCustomerDirectory, saveCustomerDirectory } from '../src/customer-directory.js'
import { prisma } from '../src/db.js'

const root = path.resolve(path.dirname(fileURLToPath(import.meta.url)), '..')

async function readJson<T>(fileName: string): Promise<T | null> {
  try { return JSON.parse(await fs.readFile(path.join(root, 'data', fileName), 'utf8')) as T }
  catch { return null }
}
...
async function main() {
  ...
  const [invoiceCount, cacheCount, directoryCount, snapshotCount] = await Promise.all([
    prisma.invoice.count(), prisma.customerSalesCache.count(),
    prisma.customerDirectoryEntry.count(), prisma.customerCrossSnapshot.count(),
  ])
  const existingRecords = invoiceCount + cacheCount + directoryCount + snapshotCount
  if (existingRecords > 0 && !replace && !resume) {
    throw new Error(
      `Neon ya contiene ${existingRecords} registros persistidos. Usa --resume para completar una importación interrumpida o --replace para sustituirlos.`,
    )
  }
  if (replace) {
  ...
  for (const extension of ['xlsx', 'xls', 'csv']) {
    ...
    try {
      const buffer = await fs.readFile(path.join(root, 'data', fileName))
